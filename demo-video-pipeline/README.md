...
This pipeline is extracted from a series of experiments on my personal blog,
[schristoph.online](https://schristoph.online):

- It began with **[HTTP 402: The 30-Year Placeholder That AI Agents Finally Activated](https://schristoph.online/blog/http-402-agents-pay/)** —
  why AI agents can finally put the web's oldest unused status code to work, paying for
  content per-article via the x402 protocol and AWS AgentCore Payments.
- The discussion that post sparked led me to actually build it:
  **[I Built the Agent That Pays](https://schristoph.online/blog/building-agent-that-pays/)** —
  a research agent with a $1 budget that autonomously discovers, evaluates, and purchases
  content from competing publishers, settling on-chain. The implementation is open at
  **[agentcore-payments-media-demo](https://github.com/stechr/agentcore-payments-media-demo)**,
  and the post includes a demo walkthrough video — one created essentially by AI, which drew
  a lot of interest of its own.
- That interest turned to the narration: in
  **[From a Generic Voice to My Own](https://schristoph.online/blog/from-polly-to-cloned-voice/)**
  I re-narrated the *same* demo in my own voice, cloned by an open-weights Qwen3-TTS model
  self-hosted on Amazon SageMaker — the Amazon Polly default → cloned-voice path you'll find here.
- As interest in *how* these demo videos are made kept growing, I extracted the demo-video
  creation pipeline into this repo for you to explore.

A deep-dive blog post on this pipeline is coming soon on
[schristoph.online](https://schristoph.online) — stay tuned. In the meantime, go build on the
published code.
...
It comes with a **neutral sample app**, a **ready-made sample script** (the Kiro agent
authored it from the app — the same flow you use for your own app), and a **script
...
Two narrated demos produced by this pipeline from the bundled sample app. **Click a
thumbnail to play** — GitHub can't embed externally-hosted video inline, so each thumbnail
links to the full MP4 (same screen recording; only the voice differs):

[![Demo — Amazon Polly voice (click to play)](docs/demo-polly-thumb.png)](https://schristoph.online/media/demo-video-pipeline-polly.mp4)

**Amazon Polly** (default) — [open video](https://schristoph.online/media/demo-video-pipeline-polly.mp4)

[![Demo — Qwen3-TTS voice, Aiden (click to play)](docs/demo-qwen-thumb.png)](https://schristoph.online/media/demo-video-pipeline-qwen.mp4)

**Qwen3-TTS named speaker** on Amazon SageMaker — [open video](https://schristoph.online/media/demo-video-pipeline-qwen.mp4)
...
In **Kiro CLI** this is the natural path — and it's exactly how the bundled sample script
was authored: the generator extracts the app's facts and emits a ready-to-paste prompt, then
**the Kiro agent writes the polished `narration.json` with its own model** (no extra AWS
call). Just ask: *"Make a script from my app in ./my-app and render it."*

```bash
# Extract facts + emit the LLM prompt (no network, no AWS):
make script-from-app          # → /tmp/draft.json (heuristic skeleton) + /tmp/prompt.txt
# From a product spec instead:
make script-from-spec SPEC=my-product.md
```

The agent reads `/tmp/prompt.txt`, writes a polished script, confirms the wording with you,
then renders it. `/tmp/draft.json` is a stdlib-only heuristic skeleton — a hand-editable
fallback for when you're *not* driving this from an agent.

Outside Kiro CLI (CI/automation) you can have the generator call an LLM directly — pass a
**current** Bedrock model id explicitly (there is no default; model ids change and expire):
...
sample-content/
  narration.json       the ready-made sample narration (agent-authored from the app)
docs/
  speaker-notes.md     human-readable companion to the script
  sample-app.png       a still of the sample app
  demo-polly-thumb.png play-button thumbnail linking to the Amazon Polly demo video
  demo-qwen-thumb.png  play-button thumbnail linking to the Qwen3-TTS demo video
pipeline/
