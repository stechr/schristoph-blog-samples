...
This answers "how do I make a script for my OWN content?" two ways:
  ...
  2. A ready-to-paste LLM PROMPT that an agent (e.g. Kiro CLI, using its own model) — or an
     optional --bedrock call with a CURRENT model id — turns into a polished narration.
...
Usage:
  ...
  # In Kiro CLI, let the agent write the script from /tmp/prompt.txt with its own model.
  # Outside an agent (CI/automation), call Bedrock directly with a CURRENT model id:
  python scripts/generate_script.py --app sample-app --bedrock \
      --model <current-bedrock-model-id> --out /tmp/script.json
...
def main():
    ...
    ap.add_argument("--model", default=None,
                    help="Bedrock model id, required with --bedrock (no default — model ids change/expire)")
    ...
    if a.bedrock and not a.model:
        print("[gen] --bedrock needs --model <current-bedrock-model-id> "
              "(no default; ids change/expire). Falling back to heuristic draft.")
